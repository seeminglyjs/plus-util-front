...
export default function UtilDetail({ authData }: Props) {
    ...
    useEffect(()=>{
        const utilNo = router.query.utilNo
        const currentNo = router.query.currentPage

        async function getUtilDetail(utilNo: number) {
            const url = `${process.env.API_BASE_URL}/util/info/detail?utilNo=${utilNo}`
            if (utilNo !== undefined && !Number.isNaN(utilNo)) {
    ...
    const enrollUtilInfo = async () => {
        ...
        if (!response.ok) {
            ...
            if (utilInfoInsertResponseDto.auth) {
                ...
                openModal("실패", "수정 중 오류가 발생했습니다.", "확인")
            }
        }
    }
                                        ...
                                        <div className="border border-gray-700 rounded-3xl py-8 px-4">
                                            <div className="py-8 px-4 mx-auto max-w-3xl">
                                                <div className="py-3 my-2 text-center">
                                                    <span className="text-xl font-bold text-white text-center mr-1">유틸 정보</span><BiEditAlt className="inline-block text-xl font-bold text-white mb-2 hover:animate-pulse"></BiEditAlt>
                                                </div>
                                                <form action="#">
